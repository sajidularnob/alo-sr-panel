...
$stmt = $conn->prepare("
    ...
    WHERE name = ? AND sr_id = ? 
    LIMIT 1
");
$stmt->bind_param("si", $shop_name, $sr_id);
$stmt->execute();
$res = $stmt->get_result();
...
if ($res->num_rows) {
    ...
    $stmt = $conn->prepare("
        INSERT INTO shops (sr_id, name, address) 
        VALUES (?, ?, ?)
    ");
    $stmt->bind_param("iss", $sr_id, $shop_name, $shop_address);
    $stmt->execute();
...
/* ORDER INSERT */
$stmt = $conn->prepare("
    INSERT INTO orders (sr_id, shop_id) 
    VALUES (?, ?)
");
